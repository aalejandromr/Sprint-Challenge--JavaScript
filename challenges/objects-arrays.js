...
class Dinosaur {
  constructor({name, diet, weigth, length, period}){
    this.name = name;
    this.diet = diet;
  ...
  roar(){
    return this.name == "tyrannosaurus" ? `RAWERSRARARWERSARARARRRR` : undefined
  }
}
...
const universities = [];
graduates.map(indivual => universities.push(indivual.university));
...
const contactInfo = [];
graduates.forEach((individual) => contactInfo.push(`${individual.first_name} ${individual.email}`));
console.log(contactInfo);


/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called uni that contains them all. Log the result. */
const uni = [];
universities.map(university => /[Uu]ni/g.test(university) ? uni.push(university) : undefined)
console.log(uni);
...
Return an array with only the animal and scientific names in it.  The individual values in the array should look like this 
...
zooAnimals.filter((animal) => animal.population < 5).map(animal => largerPopulation.push(animal))
...
populationTotal = zooAnimals.reduce((previousValue, animal) => animal.population + previousValue, 0)
